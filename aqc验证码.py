...
import time
import warnings
from init_style import init_content, style_content, location, proxy
import requests
...
def generate_trajectory(num_points, total_distance):
    ...
    if num_points == 0:
        ...
        result.append({"t": current_time, "fx": fx, "fy": random.randint(660, 680)})
    else:
...
def get_fs1():
    back_str = style_content['data']['backstr']
    angle = get_img_file()
    print('获得的旋转角度为:', angle)
    ac_c = get_ac_c(angle)
    print('ac_c:', ac_c)
...
def get_img_file():
    ...
    img_url = style_content['data']['captchalist'][0]['source']['back']['path']
...
def validate_log():
    ...
    data = {
        ...
        "as": init_content['data']['as'],
        "scene": "",
        "tk": init_content['data']['tk'],
        "ver": "2",
        "cv": "submit",
        "typeid": "spin-0",
        "fuid": get_fuid(),
    ...
    print('获得返回结果:', response.json()['data']['op'])
    print(response.json())
    print(response)
